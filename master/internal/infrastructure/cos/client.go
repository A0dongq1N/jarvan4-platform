...
func (c *Client) UploadFile(ctx context.Context, key, localPath string) error {
	_, _, err := c.cos.Object.Upload(ctx, key, localPath, &cos.MultiUploadOptions{
...
func (c *Client) DownloadFile(ctx context.Context, key, localPath string) error {
	...
	_, err := c.cos.Object.Download(ctx, key, localPath, &cos.MultiDownloadOptions{
		ThreadPoolSize: 5,
		PartSize:       10, // 每块 10MB
	})
	if err != nil {
...
func (c *Client) Exists(ctx context.Context, key string) (bool, error) {
	...
	if err != nil {
		...
		if cosErr, ok := err.(*cos.ErrorResponse); ok && cosErr.Response.StatusCode == 404 {
			return false, nil
		}
